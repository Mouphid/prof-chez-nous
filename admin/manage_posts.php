    ...
    <div class="flex min-h-screen">
        <aside class="w-64 bg-gray-900 text-white fixed h-full overflow-y-auto">
            ...
            <nav class="p-4">
                <ul class="space-y-1">
                    ...
                    <li><a href="manage_posts.php" class="flex items-center gap-3 px-4 py-3 rounded-lg bg-gray-700 text-white"><i class="fas fa-file-alt w-5"></i> Articles</a></li>
        ...
        <main class="flex-1 ml-64 p-8">
            <div class="flex justify-between items-center mb-8">
                <h1 class="text-2xl font-bold text-gray-800"><i class="fas fa-newspaper text-primary mr-2"></i> Gestion des Articles</h1>
                <?php if(has_permission('publish_articles')): ?>
                <a href="add_post.php" class="bg-primary hover:bg-indigo-700 text-white px-5 py-2.5 rounded-lg transition font-semibold flex items-center gap-2"><i class="fas fa-plus"></i> Ajouter un article</a>
                <?php endif; ?>
            </div>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-gray-50 hover:bg-gray-50">
                                ...
                                <td class="p-3">
                                    ...
                                    <?php if($can_edit): ?>
                                    <a href="edit_post.php?id=<?= $post['id_post'] ?>" class="text-emerald-600 hover:text-emerald-800 mr-2"><i class="fas fa-edit"></i></a>
                                    <?php if(has_permission('delete_any_post') || $post['id_user'] == $_SESSION['admin_id']): ?>
                                    <a href="delete_post.php?id=<?= $post['id_post'] ?>&csrf_token=<?= csrf_token() ?>" class="text-red-600 hover:text-red-800" onclick="return confirm('Supprimer cet article ?');"><i class="fas fa-trash"></i></a>
                                    <?php endif; ?>
                                    <?php endif; ?>
                                </td>
                            </tr>
